...
class RestoreAdminAccessAttributeGroupsMixin(APIManager):
    """Реализует восстановление доступа администратора (``restoreAdminAccess``).

    Путь ``POST``:
    ``.../security/attributeGroups/restoreAdminAccess``
    operationId ``Security_RestoreAdminAccessAttributeGroupCollectionSecurity``.
    """

    async def restore_admin_access_attribute_groups(
        self: "RestoreAdminAccessAttributeGroupsMixin",
        *,
        confirm: bool = False,
    ) -> None:
        """Восстанавливает доступ администратора к правам коллекции групп атрибутов (МУТАЦИЯ).
        ...
        возможность редактировать безопасность коллекции групп атрибутов. Парный read для проверки
        текущего снимка прав — :meth:`attribute_groups_security`.
        ...
        Example:
            async with IPSClient(config=config) as ips:
                await ips.restore_admin_access_attribute_groups(confirm=True)

        Notes:
            operationId ``Security_RestoreAdminAccessAttributeGroupCollectionSecurity``. Запрос без
            тела (``json={}``).
            Путь ``POST``:
            ``.../security/attributeGroups/restoreAdminAccess``
            Парный read — :meth:`attribute_groups_security`.
        """
        if confirm is not True:
            raise ValueError(
                "restore_admin_access_attribute_groups меняет права доступа; "
                "передайте confirm=True",
            )
        await self._request(
            "post",
            "/core/api/security/attributeGroups/restoreAdminAccess",
            json={},
        )
        return None
