...
class RestoreAdminAccessObjectTypeMixin(APIManager):
    """Реализует восстановление доступа администратора (``restoreAdminAccess``).

    Путь ``POST``:
    ``.../security/objectTypes/{objectTypeId}/restoreAdminAccess``
    operationId ``Security_RestoreAdminAccessObjectTypeSecurity``.
    """

    async def restore_admin_access_object_type(
        self: "RestoreAdminAccessObjectTypeMixin",
        object_type_id: int,
        *,
        confirm: bool = False,
    ) -> None:
        """Восстанавливает доступ администратора к правам конкретного типа объекта (МУТАЦИЯ).
        ...
        возможность редактировать безопасность конкретного типа объекта. Парный read для проверки
        текущего снимка прав — :meth:`object_type_security`.
        ...
        Example:
            async with IPSClient(config=config) as ips:
                await ips.restore_admin_access_object_type(1, confirm=True)

        Notes:
            operationId ``Security_RestoreAdminAccessObjectTypeSecurity``. Запрос без тела
            (``json={}``).
            Путь ``POST``:
            ``.../security/objectTypes/{objectTypeId}/restoreAdminAccess``
        ...
        if confirm is not True:
            raise ValueError(
                "restore_admin_access_object_type меняет права доступа; передайте confirm=True",
            )
        await self._request(
            "post",
            f"/core/api/security/objectTypes/{object_type_id}/restoreAdminAccess",
            json={},
        )
        return None
