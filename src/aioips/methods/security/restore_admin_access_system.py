...
class RestoreAdminAccessSystemMixin(APIManager):
    """Реализует восстановление доступа администратора (``restoreAdminAccess``).

    Путь ``POST``:
    ``.../security/system/restoreAdminAccess``
    operationId ``Security_RestoreAdminAccessSystemSecurity``.
    """

    async def restore_admin_access_system(
        self: "RestoreAdminAccessSystemMixin",
        *,
        confirm: bool = False,
    ) -> None:
        """Восстанавливает доступ администратора к правам системы (глобальные права) (МУТАЦИЯ).
        ...
        возможность редактировать безопасность системы (глобальные права). Парный read для проверки
        текущего снимка прав — :meth:`system_security`.
        ...
        Example:
            async with IPSClient(config=config) as ips:
                await ips.restore_admin_access_system(confirm=True)

        Notes:
            operationId ``Security_RestoreAdminAccessSystemSecurity``. Запрос без тела
            (``json={}``).
            Путь ``POST``:
            ``.../security/system/restoreAdminAccess``
            Парный read — :meth:`system_security`.
        """
        if confirm is not True:
            raise ValueError(
                "restore_admin_access_system меняет права доступа; передайте confirm=True",
            )
        await self._request(
            "post",
            "/core/api/security/system/restoreAdminAccess",
            json={},
        )
        return None
