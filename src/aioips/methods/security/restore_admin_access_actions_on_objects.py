...
class RestoreAdminAccessActionsOnObjectsMixin(APIManager):
    """Реализует восстановление доступа администратора (``restoreAdminAccess``).

    Путь ``POST``:
    ``.../security/actionOnObjects/restoreAdminAccess``
    operationId ``Security_RestoreAdminAccessActionsOnObjectsSecurity``.
    """

    async def restore_admin_access_actions_on_objects(
        self: "RestoreAdminAccessActionsOnObjectsMixin",
        *,
        confirm: bool = False,
    ) -> None:
        """Восстанавливает доступ администратора к правам операций над объектами (МУТАЦИЯ).
        ...
        возможность редактировать безопасность операций над объектами. Парный read для проверки
        текущего снимка прав — :meth:`actions_on_objects_security`.
        ...
        Example:
            async with IPSClient(config=config) as ips:
                await ips.restore_admin_access_actions_on_objects(confirm=True)

        Notes:
            operationId ``Security_RestoreAdminAccessActionsOnObjectsSecurity``. Запрос без тела
            (``json={}``).
            Путь ``POST``:
            ``.../security/actionOnObjects/restoreAdminAccess``
            Парный read — :meth:`actions_on_objects_security`.
        """
        if confirm is not True:
            raise ValueError(
                "restore_admin_access_actions_on_objects меняет права доступа; "
                "передайте confirm=True",
            )
        await self._request(
            "post",
            "/core/api/security/actionOnObjects/restoreAdminAccess",
            json={},
        )
        return None
