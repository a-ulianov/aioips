...
``EmptyListIfNone`` (см. [[gotchas]]). Идентификаторы-акронимы (``attributeID`` и др.)
имеют явные алиасы под точный ключ JSON. См. [[ips-object-model]] (раздел «Атрибуты»).
...
class Attribute(IPSModel):
    ...
    ``dbObjectID``, ``dB_ID``) — для них заданы явные алиасы (см. [[gotchas]]).
