...
Автогенерируемый перечень всех **723 методов** клиента `aioips` в **42 разделах**. Страница пересобирается из кода (`python scripts/gen_reference.py`), поэтому всегда соответствует реализации. Подробные описания «что/когда/как» — в docstring каждого метода (MCP-grade).

!!! note "Соглашения"
    Имена выводятся из адреса эндпоинта ([ADR-0003](../architecture.md)). Разрушающие операции защищены `confirm=True`. `object_id` в файловых/атрибутных мутациях — id рабочей копии из `object_check_out` (см. docstring).

| Раздел | Методов |
|---|---|
| [`archives`](#archives) | 1 |
| [`attribute_history`](#attribute_history) | 2 |
| [`auth`](#auth) | 4 |
| [`bridge`](#bridge) | 34 |
| [`briefcase`](#briefcase) | 10 |
| [`calendars`](#calendars) | 10 |
| [`config`](#config) | 7 |
| [`crypto_signing`](#crypto_signing) | 5 |
| [`discussions`](#discussions) | 8 |
| [`docs`](#docs) | 8 |
| [`document_editor`](#document_editor) | 19 |
| [`documents`](#documents) | 6 |
| [`editing_contexts`](#editing_contexts) | 3 |
| [`file_systems`](#file_systems) | 5 |
| [`files`](#files) | 27 |
| [`forms`](#forms) | 24 |
| [`graph_signs`](#graph_signs) | 11 |
| [`imbase`](#imbase) | 43 |
| [`improjects`](#improjects) | 24 |
| [`imviewer`](#imviewer) | 4 |
| [`licenses`](#licenses) | 2 |
| [`mail_agent`](#mail_agent) | 2 |
| [`measure_units`](#measure_units) | 2 |
| [`metadata`](#metadata) | 225 |
| [`notify`](#notify) | 1 |
| [`object_types`](#object_types) | 12 |
| [`objects`](#objects) | 70 |
| [`relation_queries`](#relation_queries) | 4 |
| [`relation_types`](#relation_types) | 2 |
| [`relations`](#relations) | 28 |
| [`samples`](#samples) | 13 |
| [`search_schemes`](#search_schemes) | 3 |
| [`security`](#security) | 63 |
| [`selection_classificators`](#selection_classificators) | 8 |
| [`settings`](#settings) | 7 |
| [`signs`](#signs) | 4 |
| [`snapshots`](#snapshots) | 4 |
| [`sso`](#sso) | 2 |
| [`table_report`](#table_report) | 4 |
| [`users`](#users) | 2 |
...
*Архивы документов: применимость настроек.*
...
*История значений атрибутов (чтение/удаление).*
...
*Авторизация: опции входа, токены (authenticate/refresh/clone).*
...
*Клиентский мост: настройки, плагины, действия запуска, temp-файлы, загрузки.*
...
*Портфель: статус, проверка метаданных, экспорт/импорт объектов.*
...
*Производственные и пользовательские календари, фильтры, запись.*
...
*Чтение параметров конфигурации сервера.*
...
*ЭЦП: настройки, сведения о подписях, хэш, создание подписей.*
...
*Обсуждения объектов: чтение и запись сообщений, изображения.*
...
*Типы/суффиксы документов, настройки, наследование.*
...
*Редактор документов: буфер, свойства, шрифты, содержимое, формулы.*
...
*Прототипы и настройки документов.*
...
*Локальные диски и каталоги сервера.*
...
*Файловые атрибуты: загрузка/прикрепление/удаление, таблицы, прототипы.*
...
*Формы и виджеты, пользователи и группы, цвета, предметные области, find*.*
...
*Настройки штампов ЭЦП (ранги/архивы/уровни и шаги ЖЦ): чтение и запись.*
...
*Справочная система: каталоги, индексы, поиск, избранное, конвертеры.*
...
*Проекты и задачи (Gantt): чтение + создание/правка/удаление/исполнение.*
...
*Данные 3D-просмотрщика (сетка/сборка/инфо/состав).*
...
*Лицензирование: идентификатор клиента, шифрование.*
...
*Почтовый агент: настройки и счётчики непрочитанного.*
...
*Метамодель: типы/атрибуты/связи, применяемость, ЖЦ, дерево типов.*
...
*Экземпляры объектов по типу и определения типов (контроллер objectTypes).*
...
*Объекты: чтение, поиск, запись, жизненный цикл, состав, версии.*
...
*Запросы состава и вхождения объекта.*
...
*Связи: чтение, атрибуты, создание/удаление, расширенный поиск.*
...
*Демо-API сообщений и значений: CRUD, выборки, файловые значения.*
...
*Схемы поиска/выборки и структура условий, правка.*
...
*Права доступа: чтение, проверки, восстановление доступа админа, запись прав.*
...
## selection_classificators

*Классификаторы выбора: чтение и включение/исключение объектов.*
...
*Настройки и права пользователя, настройки просмотра/печати, данные безопасности.*
...
*Метаданные ЭЦП: графы, ранги, параметры вывода штампа.*
...
*Снимки состава: чтение, создание, обновление, удаление.*
...
*Опции и аутентификация Kerberos.*
...
*Шаблоны и содержимое табличных отчётов.*
...
*Процессы: переменные, вложения активности (создание/прикрепление).*
